...
type ConfiguracionExtractorProps = {
    usuario: UsuarioDB | null;
    onClose: () => void;
};

const ConfiguracionExtractor = ({ usuario, onClose }: ConfiguracionExtractorProps) => {
    ...
    useEffect(() => {
        ...
        const fetchClientes = fetch("/backend/Intranet/Perfiles/Extractor/listaClientesExtractor")
            .then(res => res.json())
            .then(data => {
        ...
        const fetchClientesUsuario = fetch(`/backend/Intranet/Perfiles/Extractor/usuarioClientesExtractor?usuario=${usuario.xt_usuario}`)
            .then(res => res.json())
            .then(data => {
    ...
    const handleGuardar = async () => {
        ...
        setLoading(true);
        try {
            const response = await fetch("/backend/Intranet/Perfiles/Extractor/registraClienteUsuario", {
                method: "POST",
            ...
            if (data.success || response.ok) {
                const clientesUsuarioRes = await fetch(`/backend/Intranet/Perfiles/Extractor/usuarioClientesExtractor?usuario=${usuario.xt_usuario}`);
    ...
    const handleEliminar = async (clienteId: string) => {
        ...
        setLoading(true);
        try {
            const response = await fetch("/backend/Intranet/Perfiles/Extractor/eliminaClienteUsuario", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    usuario: usuario.xt_usuario,
        ...
        <div className="fixed inset-0 bg-gray-500/80 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-xl shadow-lg text-black">
                <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-bold">Clientes Extractor</h2>
                    <button
                                    ...
                                    <tr key={cliente.kp_clienteId} className="hover:bg-gray-100">
                                        ...
                                        <td className="px-3 py-2 border-b">
                                            <button
                                                ...
                                                onClick={() => handleEliminar(cliente.kp_clienteId)}
                                            >
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    width={20}
...
export default ConfiguracionExtractor;